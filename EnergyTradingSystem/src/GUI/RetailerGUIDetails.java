...
import EnergyAgents.RetailerAgent.Mechanism;
...
public class RetailerGUIDetails {
	...
	public RetailerGUIDetails(RetailerAgent agent) {
		myAgent = agent;
	}
	 ...
	 */
	class RetailerFrame extends JDialog {
		...
		public RetailerFrame() {
			...
			Mechanism[] mechanisms = RetailerAgent.Mechanism.values();
			cbMechanism.setModel( new DefaultComboBoxModel<>(mechanisms));
			inputPane.add(cbMechanism);
			
			for(int i=0; i<mechanisms.length; i++) {
				if ( myAgent.getNegotiationMechanism() == mechanisms[i]) {
					cbMechanism.setSelectedIndex(i);
					break;
				}
			}
