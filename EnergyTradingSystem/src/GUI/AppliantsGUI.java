...
//import GUI.RetailerGUI.AgentTableModel;
...
public class AppliantsGUI extends JFrame {
	...
	private List<AgentController> applianceLists;
	
	private JTable table;
	public AppliantsGUI(List<AgentController> applianceLists)
	{
		this.applianceLists = applianceLists;
		...
		startBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					int state = applianceLists.get(row).getState().getCode();
		...
		startAllbtn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				for(int i = 0; i < applianceLists.size(); i++) {
					try {
						int state = applianceLists.get(i).getState().getCode();
						if ( state == AgentState.cAGENT_STATE_SUSPENDED ) {
		...
		*/
		...
		setSelectedBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				int row = table.getSelectedRow();
				if ( row > -1 ) {
					JadeController.showAgentGUI(applianceLists.get(row));
				}
			}
		});
		*/
	...
	private class AgentTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int col) {
            try {
            	if ( col == 0 ) {
            		String name = applianceLists.get(row).getName();
            		return name.substring(0, name.indexOf("@"));
            	}else { // col = 1
            		if ( applianceLists.get(row).getState().getCode() == AgentState.cAGENT_STATE_SUSPENDED ) {
            			return "Stopped";
            		}else {
            			return "Started";
            		}
        ...
        public void setValueAt(Object value, int row, int col) {
        	...
        	if ( status.toLowerCase() == "start" ) {
        		try {
        			applianceLists.get(row).activate();;
				} catch (StaleProxyException e) {
					e.printStackTrace();
				}
        	}else {
        		try {
        			applianceLists.get(row).suspend();
				} catch (StaleProxyException e) {
					e.printStackTrace();
				}
        	}
